/* --------------------------------------------------------------- 
            FVC2000 enroll_samp: sample enroll_xxxx program
...
int main(int argc, char * argv[]) {   
  ...
  // Load parameters
  if (argc!=5)
    { printf("\nSyntax error.\nUse: enroll_samp <imagefile> <templatefile> <configfile> <outputfile>\n");
	  return SYNTAX_ERROR;
	}
  ...
  */
  ...
  te = fopen(templatefile,"wb");
  if (!te) return CANNOT_WRITE_TEMPLATE;
  fwrite(&mean,4,1,te);
  fclose(te);
